...
VeighNa Trader提供了微信消息通知功能。用户完成微信绑定后，程序可以通过微信实时接收系统推送消息；对于开发者，也可以通过MainEngine.send_notification接口同时向已配置的邮件和微信渠道发送通知。

在VeighNa Trader主界面菜单栏中点击【微信】按钮，会弹出【微信通知】窗口。首次使用时点击【开始绑定】，按窗口提示完成以下步骤：

- 使用微信扫描窗口中显示的二维码；
- 扫码成功后，在微信中向机器人ClawBot发送任意文字消息；
- 程序收到首条消息后会自动完成绑定，并显示Bot ID、用户ID和网关地址。

绑定成功后，可以点击【测试消息】发送一条测试通知。若微信中可以收到“VeighNa Trader 消息推送测试”，说明微信通知已经配置成功。

绑定信息会保存到.vntrader\wechat_setting.json文件中，后续启动VeighNa Trader时会自动加载。

若需要更换接收账号，可以在【微信通知】窗口中点击【重新绑定】；若不再使用微信通知，可以点击【解除绑定】清除当前绑定。

请注意，微信会话可能过期。若日志组件中出现“微信会话已过期，请通过菜单重新扫码绑定”，需要重新打开【微信】窗口完成扫码绑定。
