...
> [截图占位：fusion_ctp_01 CTP连接窗口]

连接发起后，可观察 `日志` 组件输出的状态信息。若连接成功，系统通常会继续推送账户、持仓、合约或行情相关信息。

> [截图占位：fusion_ctp_02 连接成功后的日志区域]

## 查询合约

连接成功后，建议先通过 `【帮助】->【查询合约】` 确认目标合约的交易所和代码写法是否正确。
...
若订阅后界面长期没有行情更新，应先回到合约查询页面确认代码是否填写正确，再检查当前时间段是否有有效行情推送。

> [截图占位：fusion_ctp_04 行情订阅成功界面]
...
处理建议：先检查网络状态，再确认填写的信息是否来自正确环境，并联系所属期货公司确认当前服务状态。
...
现象：连接成功后无法看到实时行情更新。  
处理建议：先确认是否已经正确订阅目标合约，再检查当前时间是否处于对应市场的有效行情时段。
